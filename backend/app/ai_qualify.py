...
Uses ZVENO free model for LLM analysis.
"""
import json
import logging
...
async def qualify_company(
    ...
    try:
        async with httpx.AsyncClient(timeout=60) as c:
            ...
            data = resp.json()
            logger.info("ZVENO qualification response status=%s body=%s", resp.status_code, json.dumps(data, ensure_ascii=False)[:500])
            if "choices" in data:
