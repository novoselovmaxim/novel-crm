...
export default function ResearchModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface w-[900px] max-h-[85vh] rounded-xl shadow-2xl flex flex-col overflow-hidden border border-muted/10">
        {/* Header */}
        ...
        <div className="px-5 py-3 border-b border-muted/10 shrink-0">
            ...
            {['brave', 'exa', 'zveno'].map(src => {
              ...
              return (
                <label key={src} className="flex items-center gap-1.5 cursor-pointer">
                  ...
                  {st && (
                    <span className={`text-[10px] ${st.status === 'ok' ? 'text-success' : st.status === 'error' ? 'text-error' : 'text-muted'}`}>
                      {st.status === 'ok' ? `✓ ${st.count}` : st.status === 'error' ? '✕' : '—'}
                    </span>
                  )}
                </label>
              )
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-bg border border-muted/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent"
              placeholder="Дополнительный запрос для поиска (необязательно)..."
            />
            <button
              onClick={runResearch}
                ...
                <div className="space-y-3">
                  ...
                  {result.scraped_texts.length === 0 ? (
                    ...
                    result.scraped_texts.map((text, i) => (
                      <div key={i} className="p-2 bg-bg rounded border border-muted/10">
                        <p className="text-[10px] text-muted mb-1">Страница {i + 1}:</p>
                        <p className="text-[11px] text-text/80 whitespace-pre-wrap line-clamp-10">{text}</p>
                      </div>
                    ))
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {(result.ai_summary || company.ai_summary) && (
                    <div className="p-2 bg-indigo-600/5 border border-indigo-600/20 rounded text-xs text-text leading-relaxed">
                      ...
                      {(result.ai_summary || company.ai_summary || '').slice(0, 500)}
                    </div>
                  )}
